...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame imageURLsGroup:(NSArray *)imageURLsGroup
{
    ...
    for (NSString *urlstr in imageURLsGroup) {
        [urlArray addObject:[NSURL URLWithString:urlstr]];
    }
    SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    cycleScrollView.imageURLsGroup = [NSMutableArray arrayWithArray:urlArray];
    return cycleScrollView;
}
...
{
    NSString   * strURL = self.imageURLsGroup[index];
    NSURL *url = (NSURL*)strURL;//[NSURL URLWithString:strURL];
    // 如果有缓存，直接加载缓存
    NSData *data = [NSData getDataCacheWithIdentifier:url.absoluteString];
      // NSData *data = [NSData getDataCacheWithIdentifier:self.imageURLsGroup[index]];
